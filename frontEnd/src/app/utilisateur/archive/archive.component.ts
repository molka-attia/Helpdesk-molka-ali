...
//   }})
  ...
  constructor(public _location: Location,private userService : AdminService, private route:ActivatedRoute,private _router:Router) {
    // Create 100 users
  ...
  onefiltrerSubmit(){
    this.userService.getTicketscloturerfiltreruser(this.formflitrer.value.groupe_id,JSON.parse(this.technicienId).userId).subscribe(
      (resultatTicket) => {
        this.fetchedTicket = resultatTicket;
       // this.dtTrigger.next();
       this.dataSource = new MatTableDataSource(this.fetchedTicket);
